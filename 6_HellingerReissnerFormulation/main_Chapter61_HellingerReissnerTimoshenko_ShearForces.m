...
%[text] %[text:anchor:H_184A76E6] The matter of transverse-shear locking and its presence in Finite Element formulation for shear-deformable structural elements is discussed in detail in Live Script [`main_Chapter4_Locking_Timoshenko_beam.mlx`](file:../4_TransverseShearLocking/4_TransverseShearLocking_TimoshenkoBeam/main_Chapter4_TransverseShearLocking_TimoshenkoBeam.mlx) at [this code section](file:../4_TransverseShearLocking/4_TransverseShearLocking_TimoshenkoBeam/main_Chapter41_TimoshenkoBeamLocking_ShearForces.mlx:T_CCE08EC4). As mentioned therein the issue is the incompatibility of the discretization spaces for the displacement and the cross-sectional rotation fields because of the use of the same basis functions for the discretization of both fields. If it is used linear shape functions then the transverse-shear deformation cannot represent zero-states by simply adding piecewise constant with piecewise linear functions, namely:
...
%[text] 
%[text] Go to [THIS](file:./main_Chapter6_HellingerReissnerFormulation_TimoshenkoBeam_Study.mlx:H_2C45B0F7) Section in the main driver script *Transverse-shear locking due incompatible discretization spaces*
%[text] Go to [NEXT](file:./main_Chapter62_HellingerReissnerTimoshenko_ConvergenceStudy.mlx:H_733CD629) Section *Convergence study on transverse shear-locking*
%[text] 
...
%[text] Change the thickness of the beam (and eventually its slenderness) using the numeric slider in this [code section](internal:M_AF2B5EFE). Try the extreme values provided in the slider
