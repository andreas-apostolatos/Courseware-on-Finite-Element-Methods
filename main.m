...
%[text] To navigate to the corresponding Live Script please select the hyperlink [`main_Chapter1_BasisFunctions.m`](file:.\1_BasisFunctions\main_Chapter1_BasisFunctions.m).
...
%[text] To navigate to the corresponding Live Script select the hyperlink [`main_Chapter2_ALinearStraightTimoshenkoBeamElement.m`](file:.\2_TimoshenkoBeam\main_Chapter2_ALinearStraightTimoshenkoBeamElement.m)`.` This Live Script takes about 40 seconds to execute.
%%
...
%[text] For the selected setup, namely, the cantilever plate subject to distributed vertical load, it can be used the same analytical solution as for the setup of the Timoshenko beam in [Chapter 2](file:.\2_TimoshenkoBeam\main_Chapter2_ALinearStraightTimoshenkoBeamElement.m). This solution is then used as in the previous chapter to vertify the solution obtained by the Finite Element Method.
%[text] To navigate to the corresponding Live Script select the hyperlink [`main_Chapter3_AShearDeformablePlateElement.m`](file:.\3_ReissnerMindlinPlate\main_Chapter3_AShearDeformablePlateElement.m)`.` The Live Script takes about 6,5 minutes to execute.
%%
...
%[text] For the Timoshenko beam problem it is used a cantilever beam as shown in [Chapter 2](file:.\2_TimoshenkoBeam\main_Chapter2_ALinearStraightTimoshenkoBeamElement.m). Moreover, it is also used as a reference the solution of the same setup using the [Euler-Bernoulli beam](https://en.wikipedia.org/wiki/Euler%E2%80%93Bernoulli_beam_theory) formulation discretized by means of the [cubic Hermitian polynomials](https://en.wikipedia.org/wiki/Cubic_Hermite_spline). Emphasis is put on the correspondance of the solutions between the two different beam theories based on the beam's slenderness.
%[text] To navigate to the corresponding Live Script select the hyperlink [`main_Chapter4_TransverseShearLocking_TimoshenkoBeam.m`](file:.\4_TransverseShearLocking\4_TransverseShearLocking_TimoshenkoBeam\main_Chapter4_TransverseShearLocking_TimoshenkoBeam.m). The Live Script takes about 40 seconds to execute.
%[text] For the Reissner-Mindlin plate problem it is used both a cantilever plate as shown in [Chapter 3](file:.\3_ReissnerMindlinPlate\main_Chapter3_AShearDeformablePlateElement.m) and a rectangular plate fixed along two of its edges while subject to a distributed vertical load.
%[text] - in what concerns the cantilever plate it is used the analytical solution presented in [Chapter 3](file:.\3_ReissnerMindlinPlate\main_Chapter3_AShearDeformablePlateElement.m) as a reference solution to evaluate the presence of transverse-shear locking,
...
%[text] To navigate to the corresponding Live Script regarding the cantilever plate select the hyperlink [`main_Chapter4_Locking_ReissnerMindlinCantileverPlate.m`](file:.\4_TransverseShearLocking\4_TransverseShearLocking_ReissnerMindlinCantileverPlate\main_Chapter4_Locking_ReissnerMindlinCantileverPlate.m). The Live Script takes about 1,5 minutes to execute.
%[text] To navigate to the corresponding Live Script regarding the rectangular plate select the hyperlink [`main_Chapter4_Locking_ReissnerMindlinSquarePlate.m`](file:.\4_TransverseShearLocking\4_TransverseShearLocking_ReissnerMindlinSquarePlate\main_Chapter4_Locking_ReissnerMindlinSquarePlate.m). The Live Script takes about a minute to execute.
...
%[text] To navigate to the corresponding Live Script regarding the weak application of Dirichlet boundary conditions select the hyperlink [`main_Chapter5_WeakBoundaryConditions_TimoshenkoBeam.m`](file:.\5_WeakDirichletBoundaryConditions\main_Chapter5_WeakBoundaryConditions_TimoshenkoBeam.m)`.` The Live Script takes about 2 seconds to execute.
%%
...
%[text] To navigate to the corresponding Live Scripts regarding the Hellinger-Reissner formulation of the Timoshenko beam problem select the hyperlink [`main_Chapter6_HellingerReissnerFormulation_TimoshenkoBeam.m`](file:.\6_HellingerReissnerFormulation\main_Chapter6_HellingerReissnerFormulation_TimoshenkoBeam.m). The Live script takes about 14 seconds to execute.
%[text] Moreover, it is also provided Live Script [`main_Chapter6_HellingerReissnerFormulation_TimoshenkoBeam_Study.m`](file:.\6_HellingerReissnerFormulation\main_Chapter6_HellingerReissnerFormulation_TimoshenkoBeam_Study.m) which performs similar studies as Live Script [`main_Chapter4_TransverseShearLocking_TimoshenkoBeam.m`](file:.\4_TransverseShearLocking\4_TransverseShearLocking_TimoshenkoBeam\main_Chapter4_TransverseShearLocking_TimoshenkoBeam.m) but it also includes insights with respect to the Hellinger-Reissner principle for the Timoshenko beam formulation and how it compares to the rest of the Finite Element formulations including also the Euler-Bernoulli beam formulation. The scripts takes about 42 seconds to execute.
%%
...
%[text] In Live Script [`main_AppexA_EfficiencyCondiderationsTimoshenkoBeam.m`](file:.\AppexA_EfficiencyConsiderations\main_AppexA_EfficiencyCondiderationsTimoshenkoBeam.m) it is shown how page-wise finite element implementation of the Timoshenko beam problem can be developed and it is also demonstrated the runtime performance gain. The script takes about one minute to execute.
%[text] Similarly, in Live Script [`main_AppexA_EfficiencyCondiderationsReissnerMindlinPlate.m`](file:.\AppexA_EfficiencyConsiderations\main_AppexA_EfficiencyCondiderationsReissnerMindlinPlate.m) it is shown how page-wise finite element implementation of the Reissner-Mindlin plate problem can be developed following the same workflow as the one demonstrated for the Timoshenko beam problem. It is herein also highlighted the substantial runtime benefit obtained by the combination of the page-wise implementation together with the use of the [`sparse`](https://www.mathworks.com/help/matlab/ref/sparse.html) matrices. This script takes about 25 minutes to execute.
